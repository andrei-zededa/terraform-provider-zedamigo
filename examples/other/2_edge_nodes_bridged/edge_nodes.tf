...
resource "zedamigo_edge_node" "ENODE_TEST_VM_AAAA" {
  ...
  extra_qemu_args = [
    ...
    "-device", "e1000,netdev=vmnet1,mac=8c:84:74:10:AA:01,bus=pci1,addr=0x0",
    "-device", "e1000,netdev=vmnet2,mac=8c:84:74:10:AA:02,bus=pci1,addr=0x1",
    "-netdev", "tap,id=vmnet1,ifname=${zedamigo_tap.TAP_A_AAAA.name},script=no,downscript=no",
    "-netdev", "tap,id=vmnet2,ifname=${zedamigo_tap.TAP_B_AAAA.name},script=no,downscript=no",
  ]
}
