# Container edge-app definition (hello-zedcloud).
#
# NOTE: This file only creates the edge-app *definition*. No edge-app-instance,
# no network-instance and no volume-instance are created here on purpose —
...
# edge-node by creating a per-edge-node edge-app-instance.
#
...
resource "zedcloud_application" "CONTAINER_APP_DEF" {
  ...
  manifest {
    ...
    #### Disabled because we deploy 3 instances on the same node and these
    #### would be a conflict.
    ####     images {
    ####       cleartext   = false
    ####       ignorepurge = true
    ...
    interfaces {
      ...
      #### Disabled because we deploy 3 instances on the same node and these
      #### would be a conflict (although the instances are connected to are
      #### network-instance of type switch).
      ####       acls {
      ####         actions {
      ####           drop       = false
      ####           limit      = false
