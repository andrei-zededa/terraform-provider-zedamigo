...
variable "edge_node_ssh_pub_key" {
  description = "Enable edge-node SSH access with the provided SSH public key. Also injected into edge-app-instances."
  sensitive   = true
  type        = string
  default     = "ssh-ed25519 AAAAC3NzBBBBBBBN5qI9j6MmXC INVALID@example.net"
}
...
# Variables for the container edge-app definition (see edge_app_container.tf).
